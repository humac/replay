...
- `js/coaching.js`: coach workspace (`/coach`) and player/family feedback (`/feedback`) mixin — roster links, timestamped notes, telestrator drawing metadata, review playlist playback, and "My Feedback" review tracking
...
- For SPA navigation, prefer the shared history helpers in `script.js`. Match URLs use slug-based paths (`/match/{slug}`). Coaching deep links are `/coach` (coach/admin capability) and `/feedback` (signed-in player/family feedback). The unified admin dashboard uses `/admin/{section}` deep links (`overview`, `matches`, `live`, `performance`, `users`, `settings`); routing + role gating live in `js/admin.js`. Legacy URLs `/admin/streams` and `/admin/system` are redirected via `LEGACY_SECTION_REDIRECTS` to `/admin/live` and `/admin/performance`. Legacy `view: 'add-match'` and `view: 'settings'` history states are still recognized in `restoreHistoryState()` and forwarded into the dashboard so back/forward keeps working across deploys.
