...
- **Outer page-shell width is uniform across the whole site.** `#app-container` (`styles.css` ~L179) is `max-width: min(100% - 1.5rem, 2200px)` with `1.25rem` padding above the existing mobile breakpoints — every view (public season, public match, coach all 4 tabs, my feedback, admin) shares one outer rhythm so navigating between them never jumps the page edges. **Don't add per-view outer caps**; use *inner* content caps instead (e.g. `.coach-grid { max-width: 1440px }`, `#admin-view { max-width: 1320px }`) when a surface needs a narrower readable measure.
