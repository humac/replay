...
- `tests/e2e/`: Playwright browser smoke checks (opt-in). Scoped with its own `package.json` and `node_modules` so the repo root stays no-build. Run from inside the folder: `cd tests/e2e && npx playwright test`. Requires the app reachable at `PLAYWRIGHT_BASE_URL` (default `http://localhost:8090`); the seed spec is `.skip()`'d until Sprint 9 of the Coach Review redesign wires real coverage. Spec files import shared helpers from `tests/e2e/_login.js` — `login()` (with retry-on-429 backoff so the auth.py 5-per-IP login rate limit doesn't flake when several specs run back-to-back), `gotoAndSettle()`, and `pickMatchWithMostNotes()`. New specs should reuse these instead of inlining their own.
