...
- `js/coaching.js`: coach workspace (`/coach`) and player/family feedback (`/feedback`) mixin — sub-tabbed `/coach` shell (Roster / Notes / Playlists / Review), Coach > Review video player + telestrator (the single note authoring surface), `<template>`-cloned note/playlist modals, and the focused feedback player modal that powers `/feedback` watch + playlist sessions without leaving the page
