...
export const apiMixin = {
    ...
    setLoggedIn() {
        ...
        const navAccount = document.getElementById('nav-account-btn');
    ...
    async loadMatches() {
        ...
        if (this.authToken && this.activeScope?.team?.id) {
            params.set('team_id', this.activeScope.team.id);
            if (this.activeScope?.season?.id) params.set('season_id', this.activeScope.season.id);
        }
        const url = `/api/matches${params.toString() ? `?${params.toString()}` : ''}`;
        try {
            const resp = params.toString()
                ? await this.authFetch(url, { headers: this.getAuthHeaders() })
                : await fetch(url);
