...
- `/admin/performance` is encoder + host telemetry colocated with the tuning knobs: tile grid + 60-s capture buttons on the left, `renderTuningKnobsCard` mounted on the right (knob change + impact in one view). Disk + diagnostics rails live below as collapsed `<details>` accordions (`#diag-errors`, `#diag-uploads`, `#diag-transcodes`, `#diag-sessions`, `#diag-audit`); each summary shows a count pill driven by `renderAdminDiagnostics` / `renderPerformanceTuning` / `renderTuningKnobsCard`.
