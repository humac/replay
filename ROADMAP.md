...
## Coaching Analysis Phase 6d-1 — Unified Coach Review: Source Modes & Creation Routing ✅ COMPLETE (2026-05-07)

Makes Coach Review the **single authoring surface** for all coaching content creation. Notes, Clips, and Playlists tabs reduce to management-only list surfaces; all creation entry points now route into Coach Review instead of opening inline forms or separate modals. Video and Tactical Board share a **single unified workspace shell** — same picker bar, same main area, same side panel — with content swapping per mode via hidden attributes.

- **Single-workspace layout** (`index.html`, `.coach-review-grid`): Video and Tactical Board share the same `.coach-review-grid` — no stacked or separate sections. Mode swap is driven by `.cr-video-only` / `.cr-board-only` `hidden` attribute toggling and `.cr-video-bar` / `.cr-board-bar` picker-bar halves (each `display: contents` so they appear as direct flex children of the shared picker bar). Video mode: picker bar shows Match / Slot / Time / Save Note / Save Clip / Focus / Shortcuts; main area shows the video player; side panel shows Telestrator toolbar + note form; timeline rail visible. Tactical Board mode: picker bar shows Event title + Save Observation; main area shows the board editor (`mountTacticalBoardEditorDirect` into `#cr-board-editor-mount`); side panel shows observation fields; timeline rail hidden.
- **Mode toggle** (`js/coaching.js` `switchCoachReviewMode()`, `_applyCoachReviewMode()`): a two-button pill strip above the picker bar switches between `video` and `tactical_board` modes. `_applyCoachReviewMode()` toggles `.cr-video-only` / `.cr-board-only` elements, clears the side panel `maxHeight` on TB entry (so the observation form renders unconstrained), and installs / uninstalls keyboard shortcuts (Space/K/J/L/arrows/S) so they only fire in video mode.
- **Direct editor mode** (`js/tactical-board.js` `mountTacticalBoardEditorDirect()`): new export that starts immediately in the editor without a preview / add-board step. Every committed stroke calls `setBoard(scene)` automatically — no Done button needed. Board tools (Player, Ball, Freehand, Arrow, Line, Zone, Label) with drag-to-draw: arrow / line / zone / freehand all use mousedown → mousemove preview → mouseup commit. Freehand accumulates points at a 0.005-distance threshold; arrow and line require a 0.01 drag minimum; zone requires a 0.02×0.02 minimum extent. Drag-to-move on any placed item updates the scene and auto-saves. Delete button and keyboard Delete / Backspace remove selected items with an auto-save. Clear uses a local in-section confirm bar (not a global modal). `freehand` shape kind added to `_VALID_BOARD_SHAPE_KINDS` in `models.py` and the validator adds 0..1 coordinate normalization + 800-point cap.
- **Tactical Board workspace** (`js/coaching.js` `renderCoachReviewTacticalBoard()`, `saveObservationFromReview()`): uses `mountTacticalBoardEditorDirect` (replacing the old preview/edit flow). Board scene held in `_coachReviewBoardScene` closure variable. Save Observation calls `POST /api/coach/notes` with `note_context: "observation"` and `tactical_board_json: _coachReviewBoardScene`. On success, fields and board reset via `_resetCrObsFields()` which re-mounts the direct editor with a blank scene.
- **Creation routing** (`goToCoachReviewWithIntent(intent)`, `_coachReviewPendingIntent`): a lightweight intent object `{ mode, intent, playerId?, defaultVisibility? }` is stashed before calling `setCoachTab('review')`. When `renderCoachReview()` fires it reads + clears the intent, calls `_applyCoachReviewMode()`, and either mounts the TB workspace (via `renderCoachReviewTacticalBoard(intent)` with player preselect + visibility default) or continues the video path. Entry points: Notes `+ New note` → `{mode:'video'}`, Notes `+ New observation` → `{mode:'tactical_board'}`, Clips `+ New clip` → `{mode:'video'}`, Roster "Add observation" clipboard icon → `{mode:'tactical_board', playerId:…, defaultVisibility:'player'}`. Existing edit paths (note / observation / clip modals) unchanged.
- **Formation presets deferred** (6d-2): the formation selector dropdown and `handleCrObsFormationChange()` are NOT in this PR. The board picker bar shows only the Event title input + Save Observation button. Formation preset authoring is the first item in Phase 6d-2.
- **Side panel simplification**: the board side panel contains only observation fields — no "Board tools" header or duplicate action buttons. All board tools live inside the board's own inline `.tb-toolbar` (via `mountTacticalBoardEditorDirect`).
- **No backend changes** beyond adding `freehand` to `_VALID_BOARD_SHAPE_KINDS` in `models.py` — Phase 6d-1 is otherwise a frontend routing + authoring-surface change. The observation `POST /api/coach/notes` endpoint from Phase 6a is reused as-is.

Browser QA (Playwright via local dev server, seeded accounts `coach1` / `family1`): mode toggle renders + switches correctly, Tactical Board mode shows pitch editor directly in the main area (no preview step) with board tools toolbar + drag-to-draw, observation fields in side panel, Video mode shows all existing controls with no regression, creation routing from Notes / Clips lands in the correct mode, Roster player preselect confirmed in Tactical Board mode, viewer (`family1`) My Feedback does not expose `coach_private_note`, dark + light themes readable, mobile 390 px no overflow. 19 screenshots in `docs/screenshots/phase-6d1-unified-coach-review/`. All 392 pytest tests pass; `node --check` and `python3 -m py_compile` clean.

**Next** (Phase 6d-2): adds 7v7 / 9v9 / 11v11 game-format selector + per-format formation presets that position player tokens on the pitch. Phase 6e then polishes viewer-side rendering: My Feedback observation context labels and Player Development Profile board integration.
...
**Next** (Phase 6d-1): UX testing of 6a/6b/6c surfaced a creation-flow correction. Phase 6d was inserted to fix it, then split into two PR-sized subphases. **Phase 6d-1 — Unified Coach Review source modes and creation routing** is the next implementation target: Coach Review becomes the single creation surface for video notes, video clips, AND tactical-board observations (Coach > Notes / Clips / Playlists / Roster reduce to management surfaces). 6d-1 reuses the existing Phase 6c board tools mostly as-is. **Phase 6d-2 — Tactical board authoring improvements and formations** then evolves the board authoring surface (drag-to-draw arrows, freehand, resizeable zone box) and ships a 7v7 / 9v9 / 11v11 game-format selector + per-format formation presets that persist as `tactical_board_json` metadata. Viewer-side polish (My Feedback + Player Development context labels and read-only board polish) is **Phase 6e**, deferred until 6d-1 + 6d-2 land so the polish targets the corrected creation flow + final tool set. See `docs/coaching-analysis-feature-roadmap.md` Phase 6 for the full spec.
