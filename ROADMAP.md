...
**Next** (Phase 6d-2): Phase 6d-1 ✅ landed the unified Coach Review source modes (Video / Tactical Board) + the routing rewrite that turns Coach > Notes / Clips / Roster into management surfaces. **Phase 6d-2 — Tactical board authoring improvements and formations** is the next implementation target: ships a 7v7 / 9v9 / 11v11 game-format selector + per-format formation presets (e.g. 4-3-3, 4-4-2 templates) that persist as `tactical_board_json` metadata, plus zone resize handles. Viewer-side polish (My Feedback + Player Development context labels and read-only board polish) is **Phase 6e**, deferred until 6d-2 lands. See `docs/coaching-analysis-feature-roadmap.md` Phase 6 for the full spec.
