...
**Next** (Phase 6e): Phase 6d-1 + Phase 6d-2 ✅ landed the unified Coach Review source modes + the tactical-board authoring tools (game-format / formation presets, zone resize handles, telestrator-aligned tools, tactical-mode keyboard shortcuts). **Phase 6e — Viewer-side My Feedback + Player Development polish** is the next implementation target: context labels for observation notes ("Practice observation", "Tactical note", "Coach observation") and read-only board polish on viewer surfaces. See `docs/coaching-analysis-feature-roadmap.md` Phase 6 for the full spec.
