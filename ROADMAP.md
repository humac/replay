...
Makes Coach Review the **single authoring surface** for all coaching content creation. Notes, Clips, and Playlists tabs reduce to management-only list surfaces; all creation entry points now route into Coach Review instead of opening inline forms or separate modals.

- **Mode toggle** (`index.html`, `js/coaching.js` `switchCoachReviewMode()`, `_applyCoachReviewMode()`): a two-button pill strip above the picker bar switches between `video` and `tactical_board` modes. Video mode shows the match/slot picker, video grid, telestrator toolbar, and Save Note + Save Clip CTAs (all existing functionality unchanged). Tactical Board mode hides the video controls and shows the inline board workspace with observation fields (event title, date, type, note title, tone, visibility, linked players, player summary, and a `<details>` block for deeper fields).
- **Tactical Board workspace** (`index.html`, `js/coaching.js` `renderCoachReviewTacticalBoard()`, `saveObservationFromReview()`): the Phase 6c `mountTacticalBoardSection` is wired inline into `#cr-board-editor-mount` so the pitch editor renders directly inside the Review tab — no nested modal. The live board scene is held in the `_coachReviewBoardScene` closure variable, read/written via `getBoard`/`setBoard` callbacks. Keyboard shortcuts (Space/K/J/L/arrows/S) are uninstalled in TB mode and reinstalled on switch back to Video mode. Save Observation calls `POST /api/coach/notes` with `note_context: "observation"` and `tactical_board_json: _coachReviewBoardScene`. On success, fields reset and the coach bundle is reloaded.
- **Creation routing** (`goToCoachReviewWithIntent(intent)`, `_coachReviewPendingIntent`): a lightweight intent object `{ mode, intent, playerId?, defaultVisibility? }` is stashed before calling `setCoachTab('review')`. When `renderCoachReview()` fires, it reads the pending intent, calls `_applyCoachReviewMode()`, and either preloads TB fields (with optional player preselect + visibility default from Roster) or continues the existing video path. Intent is consumed once and cleared.
- **Entry point changes** (`index.html`): Notes `+ New note` → `goToCoachReviewWithIntent({mode:'video', intent:'note'})`, Notes `+ New observation` → `goToCoachReviewWithIntent({mode:'tactical_board', intent:'observation'})`, Clips `+ New clip` → `goToCoachReviewWithIntent({mode:'video', intent:'clip'})`, Roster "Add observation" clipboard icon → `goToCoachReviewWithIntent({mode:'tactical_board', intent:'observation', playerId:…, defaultVisibility:'player'})`. Existing edit paths (note edit, clip edit, observation edit modals) are unchanged — only creation routes changed.
- **Styling** (`styles.css`, ~200 lines): `.coach-review-mode-toggle` pill group with `.is-active` sky-blue accent (dark) / teal (light), `.coach-review-board-workspace` two-column layout (pitch editor left, observation fields right, collapses to single column at 900 px), `.cr-obs-fields` / `.cr-obs-row` / `.cr-obs-row--split` for the inline form, themed inputs/selects/textareas in both modes, mobile-first at 390 px.
- **No backend changes** — Phase 6d-1 is purely a frontend routing + authoring-surface change. The observation `POST /api/coach/notes` endpoint from Phase 6a is reused as-is.

Browser QA (Playwright via local dev server, seeded accounts `coach1` / `family1`): mode toggle renders + switches correctly, Tactical Board mode shows board workspace + editor + 5 tone chips + observation fields, Video mode shows all existing controls with no regression, creation routing from Notes / Clips lands in the correct mode, Roster player preselect confirmed in Tactical Board mode, observation round-trip (fill → save → appears in Notes list) confirmed, viewer (`family1`) My Feedback does not expose `coach_private_note`, dark + light themes readable, mobile 390 px no overflow. Screenshots in `docs/screenshots/phase-6d1-unified-coach-review/`.

**Next** (Phase 6d-2): evolves the board authoring tools (drag-to-draw arrows, freehand strokes, resizable zone box) and adds a 7v7 / 9v9 / 11v11 game-format selector with per-format formation presets that persist as `tactical_board_json` metadata. Phase 6e then polishes viewer-side rendering: My Feedback observation context labels and Player Development Profile integration.
