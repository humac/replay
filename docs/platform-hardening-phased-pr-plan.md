...
- **Phase B — Team admin members & invites** ([#180](https://github.com/humac/replay/pull/180)). `js/coaching/team-members.js` mixin adds **Members** and **Pending Invites** cards inside Coach > Settings. Capability-gated by `membership:manage`; coaches/assistant coaches see a read-only summary. Role changes are revoke + re-grant; last-admin 409 surfaces inline. Invite composer modal sends to `/api/team/invites` with a role radio (coach / assistant_coach / team_admin / player / guardian) and a player multi-select that appears only when role=player|guardian. New `styles/team-members.css` is registered in `_STATIC_EXPORT_PATHS` and version-tagged in `settings.render_index_html()`. `/invite/{token}` landing replaces the Phase 0 placeholder with a real two-branch acceptance card (authenticated → accept; unauthenticated → create-account form).

- **Phase C — Global admin /admin/teams** ([#181](https://github.com/humac/replay/pull/181)). `js/admin-teams.js` mixin populates the `/admin/teams` sub-route reserved by Phase 0. Two-pane shell: left list of tenants with filter + new-team button; right detail panel with Overview · Seasons · Memberships sub-tabs. Backed by `/api/admin/teams*` (Phase 1.4). KPI tiles on Overview auto-refresh as sub-tab data loads. Grant-membership modal pulls `/api/users` for the user picker; revoke surfaces the backend's last-admin 409 via `notifyModal`. Boundary with Phase B is explicit: team admins do their team's day-to-day membership management inside Coach > Settings; `/admin/teams` is the global-admin override surface.
...
- **Phase E — Audit + doc closeout** ([#184](https://github.com/humac/replay/pull/184) — this PR). Cross-surface audit doc at `docs/design/admin-onboarding-audit.md` covering visual hierarchy, spacing, contrast, typography, interaction states, consistency, responsive behavior, accessibility, and the **privacy-invariant audit** (passwords / tokens / `coach_private_note` never logged or templated to viewer surfaces). All seven new surfaces pass; a small set of open items (account-tab arrow-key nav, rich rename modal, F.3 jobs strip, D.2 coach-marks, `/me` Sessions tab needs server endpoint, auto-redirect wiring) is documented as non-blocking follow-ups. Documentation closeout updates `docs/admin-guide.md` + `docs/user-guide.md` + `ROADMAP.md` + `CLAUDE.md` + `AGENTS.md`.
