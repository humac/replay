...
**Importing a roster CSV** — click **Import CSV** in the Roster header. Paste a CSV with `display_name` plus optional `player_id`, `jersey_number`, `position`/`notes`, `guardian_email`, `relationship`, and `active`. Click **Preview import** first: preview is read-only and shows which players will be created or updated, whether guardian emails will link an existing account, reuse a pending invite, or create a new guardian invite, and any row errors. Click **Commit import** only after the preview is clean. Re-importing the same roster reuses existing players/invites instead of duplicating them, and wrong-team `player_id` values are blocked. Team admins can review delivery status, resend, or revoke those invites from **Admin > People**.
