...
| `03-editor-fresh-dark.png` | Coach | Tactical board editor (empty pitch) | Editor toolbar (+ Player, + Ball, + Arrow, + Zone, + Label), Next-player-label input, Delete-selected (disabled) + Clear-board buttons, helper text, and full soccer pitch with all standard markings (touchlines, halfway, centre circle + spot, both penalty + goal areas, penalty spots + arcs, goals, corner arcs) all paint as expected | Coach editable |
